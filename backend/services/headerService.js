...
function fetchHeaders(hostname, lookup, timeoutMs = 8000) {
  return new Promise((resolve) => {
    const req = https.request(
      { host: hostname, path: "/", method: "GET", timeout: timeoutMs, rejectUnauthorized: false, ...(lookup ? { lookup } : {}) },
      (res) => {
...
async function checkHeaders(hostname, lookup) {
  const { headers, error, statusCode } = await fetchHeaders(hostname, lookup);
